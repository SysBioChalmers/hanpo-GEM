...
% newRelease
%   Prepares a new release of the hanpo-GEM model, for direct submission to
...
%Bump version number:
oldModel   = load('../ModelFiles/mat/hanpo-GEM.mat');
...
fclose(fid);
if ~contains(history,['hanpo-GEMv' newVersion ':'])
    error('ERROR: update history.md first')
end

%Load model:
model = importModel('../ModelFiles/xml/hanpo-GEM.xml');

%Include tag and save model:
model.description = ['Hansenula polymorpha-GEM_v' newVersion];

%Save model
exportForGit(model,'hanpo-GEM','..',{'mat', 'txt', 'xlsx', 'xml', 'yml'});
